%% Saves output from EnKF with different cross-localization weight factors (beta)
%
...
b_gc_YnoX = [0.1:0.1:bmax_gc, bmax_gc];
bmax_bw = bolin_wallin_beta_max(struct('rYY', rYY, 'rXX', rXX));
b_bw_YnoX = [0.1:0.1:bmax_bw, bmax_bw];
...
% Initialize arrays
RMSE_Y_YnoX_MVGC = zeros(1, length(b_gc_YnoX));
RMSE_X_YnoX_MVGC = zeros(1, length(b_gc_YnoX));
RMSE_Y_YnoX_MVBW = zeros(1, length(b_bw_YnoX));
RMSE_X_YnoX_MVBW = zeros(1, length(b_bw_YnoX));
...
for ii = 1:length(b_gc_YnoX)
    loc_params.beta = b_gc_YnoX(ii);
...
save('optimal_beta.mat', 'b_gc_YnoX', 'RMSE_Y_YnoX_MVGC', 'RMSE_X_YnoX_MVGC')
...
loc_params = struct('rYY', rYY, 'rXX', rXX); % localization parameters
for ii = 1:length(b_bw_YnoX)
    loc_params.beta = b_bw_YnoX(ii);
    [RMSE_Y, RMSE_X] = L96_EnKF_Multitrial(params, Nt, dtObs, sigma2Y, sigma2X,... 
                                Frac_Obs_Y, Frac_Obs_X, Ne, x_position, rInf, Adapt_Inf, ...
                                loc_fun_name, loc_params, True_Fcast_Err, Ntrial);
...
save('optimal_beta.mat', 'b_bw_YnoX', 'RMSE_Y_YnoX_MVBW', 'RMSE_X_YnoX_MVBW', '-append')
...
% Beta max for each function
bmax_gc = gaspari_cohn_beta_max(struct('rYY', rYY, 'rXX', rXX));
b_gc_XnoY = [0.1:0.1:bmax_gc, bmax_gc];
bmax_bw = bolin_wallin_beta_max(struct('rYY', rYY, 'rXX', rXX));
b_bw_XnoY = [0.1:0.1:bmax_bw, bmax_bw];
loc_params_askey = struct('rYY', rYY, 'rXX', rXX, 'rXY', rXY, 'nu', 1,... 
...
% Initialize arrays
RMSE_Y_XnoY_MVGC = zeros(1, length(b_gc_XnoY));
RMSE_X_XnoY_MVGC = zeros(1, length(b_gc_XnoY));
RMSE_Y_XnoY_MVBW = zeros(1, length(b_bw_XnoY));
RMSE_X_XnoY_MVBW = zeros(1, length(b_bw_XnoY));
...
loc_fun_name = 'gaspari_cohn' ; % localization function name
loc_params = struct('rYY', rYY, 'rXX', rXX); % localization parameters
for ii = 1:length(b_gc_XnoY)
    loc_params.beta = b_gc_XnoY(ii);
    [RMSE_Y, RMSE_X] = L96_EnKF_Multitrial(params, Nt, dtObs, sigma2Y, sigma2X,... 
                                Frac_Obs_Y, Frac_Obs_X, Ne, x_position, rInf, Adapt_Inf, ...
                                loc_fun_name, loc_params, True_Fcast_Err, Ntrial);
...
save('optimal_beta.mat', 'b_gc_XnoY', 'RMSE_Y_XnoY_MVGC', 'RMSE_X_XnoY_MVGC', '-append')
...
loc_fun_name = 'bolin_wallin' ;
loc_params = struct('rYY', rYY, 'rXX', rXX); % localization parameters
for ii = 1:length(b_bw_XnoY)
    loc_params.beta = b_bw_XnoY(ii);
    [RMSE_Y, RMSE_X] = L96_EnKF_Multitrial(params, Nt, dtObs, sigma2Y, sigma2X,... 
                                Frac_Obs_Y, Frac_Obs_X, Ne, x_position, rInf, Adapt_Inf, ...
                                loc_fun_name, loc_params, True_Fcast_Err, Ntrial);
...
save('optimal_beta.mat', 'b_bw_XnoY', 'RMSE_Y_XnoY_MVBW', 'RMSE_X_XnoY_MVBW', '-append')
...
% Beta max for each function
bmax_gc = gaspari_cohn_beta_max(struct('rYY', rYY, 'rXX', rXX));
b_gc_BothXY = [0.1:0.1:bmax_gc, bmax_gc];
bmax_bw = bolin_wallin_beta_max(struct('rYY', rYY, 'rXX', rXX));
b_bw_BothXY = [0.1:0.1:bmax_bw, bmax_bw];
loc_params_askey = struct('rYY', rYY, 'rXX', rXX, 'rXY', rXY, 'nu', 1,... 
...
% Initialize arrays
RMSE_Y_BothXY_MVGC = zeros(1, length(b_gc_BothXY));
RMSE_X_BothXY_MVGC = zeros(1, length(b_gc_BothXY));
RMSE_Y_BothXY_MVBW = zeros(1, length(b_bw_BothXY));
RMSE_X_BothXY_MVBW = zeros(1, length(b_bw_BothXY));
...
loc_fun_name = 'gaspari_cohn' ; % localization function name
loc_params = struct('rYY', rYY, 'rXX', rXX); % localization parameters
for ii = 1:length(b_gc_BothXY)
    loc_params.beta = b_gc_BothXY(ii);
    [RMSE_Y, RMSE_X] = L96_EnKF_Multitrial(params, Nt, dtObs, sigma2Y, sigma2X,... 
                                Frac_Obs_Y, Frac_Obs_X, Ne, x_position, rInf, Adapt_Inf, ...
                                loc_fun_name, loc_params, True_Fcast_Err, Ntrial);
...
save('optimal_beta.mat', 'b_gc_BothXY', 'RMSE_Y_BothXY_MVGC', 'RMSE_X_BothXY_MVGC', '-append')
...
loc_fun_name = 'bolin_wallin' ;
loc_params = struct('rYY', rYY, 'rXX', rXX); % localization parameters
for ii = 1:length(b_bw_BothXY)
    loc_params.beta = b_bw_BothXY(ii);
    [RMSE_Y, RMSE_X] = L96_EnKF_Multitrial(params, Nt, dtObs, sigma2Y, sigma2X,... 
                                Frac_Obs_Y, Frac_Obs_X, Ne, x_position, rInf, Adapt_Inf, ...
                                loc_fun_name, loc_params, True_Fcast_Err, Ntrial);
...
save('optimal_beta.mat', 'b_bw_BothXY', 'RMSE_Y_BothXY_MVBW', 'RMSE_X_BothXY_MVBW', '-append')
